...
const extendPlus = (client: PoolClient, accountId: string, days: number) =>
  client.query(
    `update accounts set plus_until = greatest(coalesce(plus_until, now()), now()) + make_interval(days => $2) where id = $1`,
    [accountId, days],
  );
...
export async function settleFriend(friendId: string, appOrigin?: string): Promise<Settled | null> {
  ...
  const settled = await transaction(async (client) => {
    ...
    await extendPlus(client, friendId, days);
    if (referrerId && referrerDays) await extendPlus(client, referrerId, referrerDays);
    await client.query(
      'update friend_referrals set rewarded_at = now(), friend_days = $2, referrer_days = $3 where friend_id = $1',
      [friendId, days, referrerDays],
    );
    return { friendDays: days, referrerId, referrerDays };
  });

  // Tell the one who invited, by email where there is mail. Their app says
  // so too, the next time they open the invite card.
  if (settled?.referrerId && settled.referrerDays && appOrigin && canSendMail()) {
    const to = await query<{ email: string }>('select email from accounts where id = $1', [settled.referrerId]);
    if (to[0]) {
      const link = `${appOrigin}/`;
      void compose('friend-reward', to[0].email, { days: String(settled.referrerDays), app_link: link }, originOf(link))
...
export async function friendsView(accountId: string): Promise<FriendsView> {
  ...
  const [totals, mine, fresh] = await Promise.all([
