...
export function suggestedQuestions(meals: MealEntry[], targets: Targets, today: string, hour: number, count = 3): string[] {
  ...
  if (hour >= 16 && hour < 21 && kcalLeft >= 300) add(`What should I have for dinner with ${kcalLeft.toLocaleString('en-GB')} kcal left?`);
  ...
  if (logged.length >= 3) {
    ...
    if (fibre < targets.fibre * 0.7) add('How can I eat more fibre?');
  }
