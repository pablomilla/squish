...
export default function PackTile({ contents, dark, onTap }: { contents: PackContents; dark: boolean; onTap: () => void }) {
  ...
  return (
    <button type="button" className="pack-tile look--locked" onClick={onTap} aria-label={`${contents.name}: ${contents.words}. Not on sale yet.`}>
      ...
      <span className="pack-text">
        ...
        <span className="tile-note">
          {contents.count} {contents.count === 1 ? 'item' : 'items'} · coming soon
        </span>
      </span>
    </button>
  );
