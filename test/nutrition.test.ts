...
test('scaling a portion scales every nutrient', () => {
  const scaled = scaleNutrients({ calories: 200, protein: 10, carbs: 20, fat: 5, fibre: 2, satFat: 1.4, sugar: 4, freeSugar: 3, sodium: 100, micros: { iron: 2 } }, 1.5);
  assert.deepEqual(scaled, { calories: 300, protein: 15, carbs: 30, fat: 7.5, fibre: 3, satFat: 2.1, sugar: 6, freeSugar: 4.5, sodium: 150, micros: { iron: 3 } });
});
...
test('scaling something with no saturates figure does not invent one', () => {
  ...
  assert.equal(scaled.freeSugar, undefined);
  assert.equal(scaled.micros, undefined);
});
